...
def run_single_start(
    seed: int, base_params: np.ndarray, cfg: PDEConfig, data: PatientData
    ...
    res = minimize(
        ...
        x0,
        method="Powell",
        options={"maxiter": int(cfg.maxiter), "maxfev": int(cfg.maxfev), "disp": False},
    )
...
def multistart_fit_pde(
    base_params: list[float] | np.ndarray, cfg: PDEConfig, data: PatientData
) -> np.ndarray:
